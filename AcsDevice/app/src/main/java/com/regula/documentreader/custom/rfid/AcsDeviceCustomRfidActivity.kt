...
import com.regula.documentreader.api.internal.viewmodel.mdl.MdlViewModel
...
class AcsDeviceCustomRfidActivity : BaseCustomRfiActivity() {
    ...
    fun prepareBle() {
        if (!BluetoothSettingsHelper.isBluetoothSupported(this@AcsDeviceCustomRfidActivity)) {
            completeActivity()
            return
        }
        if (!isBluetoothPermissionsReady(this@AcsDeviceCustomRfidActivity)) {
            return
        }
        if (!isBluetoothSettingsReady(this@AcsDeviceCustomRfidActivity)) {
            return
        }
        initBle()
    }
    ...
    fun isBluetoothPermissionsReady(activity: Activity): Boolean {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
            if (ContextCompat.checkSelfPermission(
                ...
                ActivityCompat.requestPermissions(
                    ...
                    BluetoothPermissionHelper.BLE_ACCESS_PERMISSION
                )
                return false
            }
            if (ContextCompat.checkSelfPermission(
    ...
    fun isPermissionDenied(activity: Activity?, permission: String): Boolean {
        if (activity == null) {
            ...
            return PermissionUtil.isPermissionsDenied(
                activity,
                permission
            ) || !PermissionUtil.isPermissionGranted(activity, permission)
        }
    }
    ...
    fun isBluetoothSettingsReady(activity: Activity?): Boolean {
        if (!isBluetoothEnabled(activity)) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S
                && MdlViewModel.isPermissionDenied(
                    activity,
                    Manifest.permission.BLUETOOTH_CONNECT
                )
            ) {
                ...
                BluetoothSettingsHelper.requestEnableBluetooth(activity)
                return false
            }
        } else if (!BluetoothSettingsHelper.isLocationServiceEnabled(activity)) {
            BluetoothSettingsHelper.requestEnableLocationService(activity)
            return false
        } else return true
    }
    ...
    override fun onRequestPermissionsResult(
        ...
        if (requestCode != BluetoothPermissionHelper.BLE_ACCESS_PERMISSION)
            return

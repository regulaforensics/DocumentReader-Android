...
class SuccessfulInitActivity : AppCompatActivity() {
    ...
    private var irImage: ImageView? = null
    ...
    private fun showIrImage(documentReaderResults: DocumentReaderResults?) {
        val irDocumentReaderGraphicField = documentReaderResults?.getGraphicFieldByType(
            eGraphicFieldType.GF_DOCUMENT_IMAGE,
            eRPRM_ResultType.RPRM_RESULT_TYPE_RAW_IMAGE, 0, eRPRM_Lights.RPRM_Light_IR_Full
        )
        if (irDocumentReaderGraphicField != null && irDocumentReaderGraphicField.bitmap != null)
            irImage?.setImageBitmap(resizeBitmap(irDocumentReaderGraphicField.bitmap))
    }
    ...
    private fun initViews() {
        ...
        irImage = findViewById(R.id.irImageView)
